...
export const GetDefaultNotes=(Notes:Note[],istext:boolean)=>{
    ...
    return currentExistingNotes.map((note)=>{
         ...
         return (istext)? {id:note.id,noteInfo:{title:note.noteInfo.title,relatedTags:tag,noteText:note.noteInfo.noteText}}:{id:note.id,noteInfo:{title:note.noteInfo.title,relatedTags:tag}} as NoteV2
